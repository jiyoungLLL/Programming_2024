...
int getWinner(void)
{
    int max_coins = -1, winner = -1;
    for (int i = 0; i < N_PLAYER; i++)
    {
        if (player_status[i] == PLAYERSTATUS_LIVE && player_coin[i] > max_coins)
        {
            max_coins = player_coin[i];
            winner = i;
        }
    }
    return winner;
...
// ----- EX. 4 : player ------------
        ...
        if (player_position[turn] == board_stepShark()){
            printf("Shark caught %s!\n", player_name[turn]);
